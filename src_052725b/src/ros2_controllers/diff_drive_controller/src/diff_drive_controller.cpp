  ...
  const rclcpp::Time & time, const rclcpp::Duration & /*period*/)
{
  ...
  //const std::shared_ptr<Twist> command_msg_ptr = *(received_velocity_msg_ptr_.readFromRT());
  ...
  const rclcpp_lifecycle::State &)
{
  ...
  if (publish_limited_velocity_)
  {
    limited_velocity_publisher_ = get_node()->create_publisher<TwistStamped>(
      DEFAULT_COMMAND_OUT_TOPIC, rclcpp::SystemDefaultsQoS().durability_volatile());
  ...
  velocity_command_subscriber_ = get_node()->create_subscription<TwistStamped>(
    DEFAULT_COMMAND_TOPIC, rclcpp::SystemDefaultsQoS().durability_volatile(),
  ...
  odometry_publisher_ = get_node()->create_publisher<nav_msgs::msg::Odometry>(
    DEFAULT_ODOMETRY_TOPIC, rclcpp::SystemDefaultsQoS().durability_volatile());
  ...
  odometry_transform_publisher_ = get_node()->create_publisher<tf2_msgs::msg::TFMessage>(
    DEFAULT_TRANSFORM_TOPIC, rclcpp::SystemDefaultsQoS().durability_volatile());
...
void DiffDriveController::halt()
{
  const auto halt_wheels = [this](auto & wheel_handles)
  {
    ...
    {
      const bool ok = wheel_handle.velocity.get().set_value(0.0);
      if (!ok)
      {
        RCLCPP_WARN(get_node()->get_logger(), "Failed to set wheel velocity to 0.");
      }
    }
  };
